...
resource "aws_instance" "logger" {
  ...
  ami           = coalesce(var.logger_ami, element(concat(data.aws_ami.logger_ami.*.image_id, tolist("")), 0))
...
resource "aws_instance" "dc" {
  ...
  ami = coalesce(var.dc_ami, element(concat(data.aws_ami.dc_ami.*.image_id, tolist("")), 0))
...
resource "aws_instance" "wef" {
  ...
  # Uses the local variable if external data source resolution fails
  ami = coalesce(var.wef_ami, element(concat(data.aws_ami.wef_ami.*.image_id, tolist("")), 0))
...
resource "aws_instance" "win10" {
  ...
  # Uses the local variable if external data source resolution fails
  ami = coalesce(var.win10_ami, element(concat(data.aws_ami.win10_ami.*.image_id, tolist("")), 0))
