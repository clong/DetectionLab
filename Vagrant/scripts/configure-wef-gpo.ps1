...
$gPLinks = Get-ADOrganizationalUnit -Server "dc.deathstar.local" -Identity $OU -Properties name,distinguishedName, gPLink, gPOptions
...
$OU = "ou=Domain Controllers,dc=deathstar,dc=local"
$gpLinks = $null
$gPLinks = Get-ADOrganizationalUnit -Server "dc.deathstar.local" -Identity $OU -Properties name,distinguishedName, gPLink, gPOptions
$GPO = Get-GPO -Name $GPOName
If ($gPLinks.LinkedGroupPolicyObjects -notcontains $gpo.path)
